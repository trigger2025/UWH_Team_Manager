...
- **Results Page Integration**: Completed Tournaments section appears at the bottom of Session History when history exists; each entry is a collapsible card showing date, team count, fixture count, winner badge, final standings (points), and per-team expandable sections showing `ratingBefore→ratingAfter (+delta)` rating change per player. No per-fixture breakdown
