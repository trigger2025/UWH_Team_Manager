...
- **Round-Based 2-Pool Scheduling**: Schedule uses true round-robin rounds (`buildScheduleRounds`). In 1-pool mode, each game is its own time slot (Bye = all non-playing teams for that slot). In 2-pool mode, each ROUND is one time slot with two games running simultaneously — first game goes to Pool A, second to Pool B; `round.bye` from the polygon rotation identifies the sitting-out team
